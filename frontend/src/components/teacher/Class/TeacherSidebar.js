...
function TeacherSidebar({ classInfo: propClassInfo }) {
  const location = useLocation();
  const matchClass = matchPath('/teacher/class/:classId/*', location.pathname);
  const classId = matchClass?.params.classId || '1234Absdth';
  ...
  // Fetch class information if not provided as prop
  useEffect(() => {
    const fetchClassInfo = async () => {
      try {
        const data = await api.classes.getById(classId);
        setClassInfo(data.class);
        // Lưu vào AuthStorage để tránh mất dữ liệu
        ...
        // Thử lấy từ AuthStorage trước khi dùng fallback
        ...
        if (cachedClassInfo && cachedClassInfo.id == classId) {
          ...
          // Tạo fallback với classId từ URL
          setClassInfo({
    ...
    // If classInfo is provided as prop, use it
    if (propClassInfo) {
    ...
    } else if (classId && classId !== '1234Absdth') {
      fetchClassInfo();
    } else {
      setLoading(false);
    }
  ...
  const menuItems = [
    { label: 'Bảng tin', link: `/teacher/class/${classId}/announcement` },
    { label: 'Lịch học', link: `/teacher/class/${classId}/teacher-schedule` },
    { label: 'Thành viên', link: `/teacher/class/${classId}/members` },
    { label: 'Bài tập', link: `/teacher/class/${classId}/assignments` },
    { label: 'Bảng điểm', link: `/teacher/class/${classId}/grades` },
    { label: 'Tài liệu', link: `/teacher/class/${classId}/materials` },
  ];

  const isActive = (link) => {
    if (link.includes('assignments')) {
      return location.pathname.startsWith(link) || location.pathname.startsWith('/teacher/upload-assignment');
    } else if (link.includes('members')) {
      return location.pathname.startsWith(link) || location.pathname.startsWith('/teacher/members/');
    } else if (link.includes('grades')) {
      return location.pathname.startsWith(link) || location.pathname.startsWith('/teacher/student-score');
    }
    return location.pathname.startsWith(link);
  };

  return (
    <div className="class-teacher-sidebar">
      <div className="class-teacher-sidebar__class-info">
        <div className="class-teacher-sidebar__class-name">
          {loading ? 'Đang tải...' : classInfo.name}
        </div>
        <div className="class-teacher-sidebar__class-code">
          Mã lớp: {loading ? 'Đang tải...' : (classInfo.class_code || classInfo.code)?.toUpperCase() || 'N/A'}
        </div>
      </div>
      <nav className="class-teacher-sidebar-nav">
        {menuItems.map((item) => (
          <Link
            to={item.link}
            key={item.label}
            className={`class-teacher-sidebar-nav__item ${isActive(item.link) ? 'active' : ''}`}
          >
            <div className="class-teacher-sidebar-icon">
              {getIconForLabel(item.label, isActive(item.link))}
            </div>
            {item.label}
          </Link>
        ))}
      </nav>
      <div className="class-teacher-sidebar-divider"></div>
      <Link
        to={`/teacher/class/${classId}/settings`}
        className={`class-teacher-sidebar-settings ${isActive(`/teacher/class/${classId}/settings`) ? 'active' : ''}`}
      >
        <div className="class-teacher-sidebar-icon">
          {getIconForLabel('Cài đặt lớp học', isActive(`/teacher/class/${classId}/settings`))}
        </div>
        Cài đặt lớp học
      </Link>
    </div>
...
function getIconForLabel(label, isActive) {
  const color = isActive ? '#1976d2' : '#555';
  switch(label) {
    case 'Bảng tin':
      return (
    ...
    case 'Bảng điểm':
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ color }}>
          <path d="M9 17H5V21H9V17Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M13 3H9V21H13V3Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M19 7H15V21H19V7Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      );
    case 'Tài liệu':
      return (
